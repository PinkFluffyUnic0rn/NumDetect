...
int imghsplit(struct nd_image *img, struct nd_image *imgparts, int partc)
{
	int partn;
...
int imgvsplit(struct nd_image *img, struct nd_image *imgparts, int partc)
{
	int partw;
	int partn;
...
int imgfindlines(struct nd_image *img, double *lines, int *linescount,
...
int getparallel(double *lines1, int lines1count,
	double *lines2, int lines2count,
	int *li1, int *li2)
{
	int minisum;
	int i1, i2;
...
// split image into four parts by a vertical axis
	...
	if (imgvsplit(&img, imgparts, 5) < 0) {
		fprintf(stderr, "nd_imgvsplit: %s\n",
			nd_strerror(nd_error));
		return 1;
	}

	linescount = 100;
	linesleft = malloc(sizeof(double) * 2 * linescount);
	linesright = malloc(sizeof(double) * 2 * linescount);
