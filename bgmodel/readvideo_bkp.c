...
int scanloop(struct nd_image *img)
{
	...
	copyw = 640;
	copyh = img->h * copyw / img->w;

	if (bg_allocbgmodel(&bgm, copyw, copyh) < 0)
		return (-1);

	framestoinit = INITFRAMESCOUNT;
	c = 0;

	if ((initframes = malloc(sizeof(struct nd_image) * INITFRAMESCOUNT))
		== NULL) {
	...
	while (1) {
		...
		c = (c < INITFRAMESSTEP - 1) ? c + 1 : 0;

		if (c == 0 && framestoinit > 0) {
			--framestoinit;

			if (nd_pslock(1) < 0)
				return (-1);

			if (nd_imgscalebilinear(img, copyw / (double) img->w,
				copyh / (double) img->h,
				initframes + framestoinit) < 0)
				return (-1);
		...
		if (framestoinit == 0) {
			...
			if (bg_initbgmodel(initframes, INITFRAMESCOUNT, &bgm,
				5, 0.3, 0.05) < 0)
				return (-1);
		}
		if (framestoinit < 0) {
			...
			if (bg_updatebgmodel(&imgcopy, &bgm) < 0)
				return (-1);
		
			nd_imgdestroy(&imgcopy);

			int *isfglowres;
			int lrw, lrh;
			int sqrsz;

			sqrsz = MAX(bgm.w, bgm.h) / 25;

			lrw = bgm.w / sqrsz;
			lrh = bgm.h / sqrsz;

			isfglowres = malloc(sizeof(double) * lrw * lrh);

			struct bg_rect *r;
			int rc;
	
			bg_isforegroundtolowres(bgm.isforeground, bgm.w, bgm.h,
				isfglowres, lrw, lrh);
			
			bg_isforegroundrectgroup(isfglowres, lrw, lrh,
				&r, &rc);
			...
			for (y = 0; y < drawimg->h; ++y)
				...
				/*
					...
					celly = y * bgm.h / drawimg->h;
					cellx = x * bgm.w / drawimg->w;
				*/
					pix = drawimg->data + (y * drawimg->w + x) * 3;
				//	if (bgm.isforeground[celly * bgm.w + cellx] == 0)
			...
			for (i = 0; i < rc; ++i) {
				...
				y0 = r[i].y0 * drawimg->h / lrh;
				y1 = (r[i].y1 + 1) * drawimg->h / lrh;

				x0 = r[i].x0 * drawimg->w / lrw;
				x1 = (r[i].x1 + 1) * drawimg->w / lrw;
...
/*
			for (y = 0; y < lrh; ++y)
				for (x = 0; x < lrw; ++x) {
					...
					if (isfglowres[y * lrw + x] == 0)
						continue;

					y0 = y * drawimg->h / lrh;
					y1 = (y + 1) * drawimg->h / lrh;

					x0 = x * drawimg->w / lrw;
					x1 = (x + 1) * drawimg->w / lrw;
